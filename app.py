...
with st.expander("使い方"):
    st.markdown(
        """
        1. 左のサイドバーで検索語を入れて **検索**
        2. 候補から動画を選ぶ
        3. サムネと情報を確認する
        4. 左サイドバーの **初期ループ設定** を必要に応じて指定する
        5. **動画を選択** を押してプレイヤーに読み込む
        6. 動画を再生しながら、プレイヤー内の **ここを開始にする** / **ここを終了にする** で微調整する
        7. **ループ開始** を押す
        ...
        **ここを終了にする** を押した場合、開始位置は変更されません。
        ...
        `yt-dlp` を使って検索結果の情報だけ取得しています。動画ファイルはダウンロードしません。
